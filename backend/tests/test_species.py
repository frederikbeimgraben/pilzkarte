...
def test_no_shipped_stem_is_hollow_and_solid_at_once() -> None:
    for slug, profile in read_profiles(DATA / "arten").items():
        assert not {StemFeature.HOLLOW, StemFeature.SOLID} <= set(profile.stem_features), slug
...
def test_a_stem_is_hollow_or_solid_but_not_both() -> None:
    # "Jung voll, spaeter hohl" ist eine Entwicklung. Die Liste kennt keine
    # Phase, darum steht dort der reife Zustand und nicht beide.
    with pytest.raises(ValidationError, match="hohl oder voll"):
        Profile.model_validate(_profile_base() | {"stielmerkmale": ["hohl", "voll"]})
