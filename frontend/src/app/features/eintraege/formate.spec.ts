import { alsDatum, hektarText, isoDatum, kurzesDatum, langesDatum, ortText } from './formate';
...
describe('Formate', () => {
  ...
  it('fällt bei einem unvollständigen Datum auf den ersten Januar zurück', () => {
    expect(isoDatum(alsDatum('2026'))).toBe('2026-01-01');
  });

  it('schreibt das lange Datum wie im Artboard Fund', () => {
    expect(langesDatum('2026-09-06', 'de')).toBe('6. September 2026');
  });

  it('schreibt heute als „Heute“ und sonst kurz', () => {
    expect(kurzesDatum('2026-09-10', 'de', '2026-09-10', 'Heute')).toBe('Heute');
    expect(kurzesDatum('2026-09-06', 'de', '2026-09-10', 'Heute')).toContain('6.');
  });

  it('schreibt den Ort mit vier Stellen', () => {
    expect(ortText(48.5203, 9.0511, 'de')).toEqual({ lat: '48,5203', lon: '9,0511' });
  });

  it('schreibt eine kleine Fläche mit einer Stelle und eine große ohne', () => {
    expect(hektarText(4.25, 'de')).toBe('4,3');
    expect(hektarText(42.4, 'de')).toBe('42');
  });
});
